...
if($sessionUserId == 0){//extant session no
  if($action != "login"){//not login and no session so error
    ...
    if(count($wpLoginRecords)==0){// post login has record no
      ...
      if(count($emailRecords)>0){ // don't allow new user with extant email
        $thisRecord=$emailRecords[0];
        $correctLogin=$thisRecord['wpLogin'];
        array_push($errorStack, '"ERROR '.$wpUserMeta['wpUserEmail'].' already in use. Log out, restart your browser and sign in as '.$correctLogin.'."');
        }
      else{
      ...
      if($loggedInUserObject['email'] != $wpUserMeta['wpUserEmail']){
        ...
        if(count($emailRecords)>0){// if yes, fail message
          ...
          $loggedInUserObject['email'] = $wpUserMeta['wpUserEmail'];
  ...
  if($_POST['hashAction']=="careerSeekerToken"){
    updateUserKeyValue($loggedInUserObject["id"], "careerSeekerToken", $_POST['hashId']);
    array_push($debugStack, '"EXTANT SESSIOn '.$loggedInUserObject["id"].' careerSeekerToken set to '.$_POST['hashId'].'"');
...
if(count($errorStack)==0){// can return data
  array_push($messagesStack, '"Welcome to the new My Core Passion. If you encounter any problems contact<br /> Lori Lorenz at 612-702-2514."');

  $loginString='"login":[' . implode(", ", $loginStack) . ']';
  array_push($returnStack, $loginString);
  $loggedInUserObject["clientCount"]="0";
  $loggedInUserObject['proceedToItemId']="0";  

    $openReceiptsList=returnOpenReceipts();
    $allOpenReceiptsString='"allOpenReceipts":'.json_encode($openReceiptsList);
    array_push($returnStack, $allOpenReceiptsString);
  ...
  if($action=="updateUserProfile"){
    ...
    $qString.="`pdfMethod` = '".$_POST['pdfMethod']."'";
    $qString.=" WHERE id = ".$sessionUserId;
    ...
    $loggedInUserObject['pdfMethod']=$_POST['pdfMethod'];
...
*/
  ...
  if($sessionIsAdmin==1){ // admin only mod consultants and search
    ...
    if($action=="login"){// admin login
      ...
      $loggedInUserObject["clientCount"]="".count($myUsersList);
  ...
  if($action=="grant"){
    ...
    if($_POST['disposition']=="self"){
      insertItem($_POST['sku'], $_POST['productId'], 'grant', $sessionUserId, 0, $_POST['targetUserId'], '');
      //insertItem($sku,   $productId,   $receiptTransaction, $creatorUserId, $consultantUserId, $consumerUserId,$inviteEmail) 
      array_push($messagesStack, '"Granted '.$_POST['units'].' of product '.$_POST['productId'].' to user '.$_POST['targetUserId']. '"');
      }
...
// recovery actions
  if($action=="createRecoveredInvites"){
    $recovered=0;
    for($u=0; $u<$_POST['units']; $u++){
      insertItem("p_1_cpa_i", "1", "recovered".$u, $loggedInUserObject['id'], $loggedInUserObject['id'], 0, "");   
      $recovered++; 
      }
    $iqResult = returnUnusedInvitesOfConsultant($sessionUserId);
    $loggedInUserObject['unusedInvites']=count($iqResult);
    array_push($messagesStack, "createRecoveredInvites units=".$_POST['units']. " recovered=".$recovered . " iqResult=".count($iqResult));

    }
  if($action=="storeRecovered"){
    // create items
    array_push($debugStack, '"recover"');
    insertItem('p_1_cpa_s', '1', 'recover', $sessionUserId, $_POST['consultantUserId'], $sessionUserId, '');
    array_push($messagesStack, '"Recovered Item"');
    $productsList=returnProducts();
    $productsString='"productsList":' .json_encode($productsList);
    array_push($returnStack, $productsString);
    //then store state
    ...
    $itemArray=returnItemsWhere("id", $last_id);
    array_push($messagesStack, '"lastId='+$last_id+'"'); 
    if(count($itemArray)>0){
      $item=$itemArray[0];//just use first record to get member metadata
      updateItemKeyValue($last_id, "stateObj", $_POST['stateObj']);
      updateItemKeyValue($last_id, "updatedYYYYMMDD", $_POST['yyyymmdd']);
      updateItemKeyValue($last_id, "consumed", $_POST['consumed']);
      updateItemKeyValue($last_id, "completed", $_POST['completed']);

      updateItemKeyValue($last_id, "ethnicity_id", $_POST["ethnicity_id"]);
      updateItemKeyValue($last_id, "gender_id", $_POST["gender_id"]);
      updateItemKeyValue($last_id, "age_id", $_POST["age_id"]);
      updateItemKeyValue($last_id, "career_id", $_POST["career_id"]);


      updateUserKeyValue($sessionUserId, "consultantUserId", $_POST['consultantUserId']);
      $loggedInUserObject["consultantUserId"]= $_POST['consultantUserId'];

      updateUserKeyValue($sessionUserId, "isAssessed", 1);
      $loggedInUserObject["isAssessed"]= 1;

      updateUserKeyValue($sessionUserId, "assessmentIsBlind", $_POST['isBlind']);
      $loggedInUserObject["assessmentIsBlind"]= $_POST['isBlind'];

      updateUserKeyValue($sessionUserId, "ethnicity_id", $_POST['ethnicity_id']);
      $loggedInUserObject["ethnicity_id"]= $_POST['ethnicity_id'];

      updateUserKeyValue($sessionUserId, "gender_id", $_POST['gender_id']);
      $loggedInUserObject["gender_id"]= $_POST['gender_id'];

      updateUserKeyValue($sessionUserId, "age_id", $_POST['age_id']);
      $loggedInUserObject["age_id"]= $_POST['age_id'];

      updateUserKeyValue($sessionUserId, "career_id", $_POST['career_id']);
      $loggedInUserObject["career_id"]= $_POST['career_id'];

      if($_POST['completed']=="1"){
        array_push($messagesStack, '"Item Completed '.$_POST['yyyymmdd'].'"');

        updateUserKeyValue($sessionUserId, "stateObj", $_POST['stateObj']);
        updateUserKeyValue($sessionUserId, "assessmentItemId", $last_id);
        $loggedInUserObject["stateObj"]= $_POST['stateObj'];
        $loggedInUserObject["assessmentItemId"]= $last_id;
        }
      else{
        array_push($messagesStack, '"Item Incomplete '.$_POST['yyyymmdd'].'"');
        }
      }
    else{
      array_push($errorStack, '"ERROR storeRecovered '.$last_id.'"');
      }
    }
...
*/
      ...
      if($item['consumerUserId']==$sessionUserId){
        ...
        if($_POST['completed']==1){
          ...
          if($loggedInUserObject["consultantUserId"] != 0){// notify consultant
            ...
            $cleanName = str_replace("&reg", "", $itemName);

            array_push($debugStack, '"itemName = '.$itemName.'"');
            array_push($debugStack, '"cleanName = '.$cleanName.'"');
            ...
            $headers = 'From: invitation@corepassion.com';
            $subject=$loggedInUserObject["firstName"]. " " .$loggedInUserObject["lastName"] . ' (' . $loggedInUserObject["wpLogin"]. ') Completed '.$cleanName;
  ...
  $loggedInUserObject['sessionHashAction']=$sessionHashAction;
  $loggedInUserObject['sessionHashId']=$sessionHashId;
...
*/
  if($receipt['disposition']==""){
    array_push($debugStack, '"bundle 0 '.$receipt['bundle0'].' id '.$receipt['bundle0id'].'"');
    
    if($receipt['bundle0id']==0){
      array_push($debugStack, '"bundle 0 id is 0, huh"');
      array_push($messagesStack, "You purchased one ".$receipt['name']);
      insertItem($receipt['sku'], $receipt['id'], $receipt['x_trans_id'], $loggedInUserObject['id'], $loggedInUserObject['consultantUserId'], $loggedInUserObject['id'], "");    
      updateReceiptKeyValue($receipt['x_trans_id'], "itemCreated", 1);
      }
    else{
      array_push($debugStack, '"bundle 0 not 0"');
      //insert bundled skus
      array_push($messagesStack, "You purchased ".$receipt['name']);

      insertItem($receipt['bundle0'], $receipt['bundle0id'], $receipt['x_trans_id'], $loggedInUserObject['id'], $loggedInUserObject['consultantUserId'], $loggedInUserObject['id'], "");
      insertItem($receipt['bundle1'], $receipt['bundle1id'], $receipt['x_trans_id'], $loggedInUserObject['id'], $loggedInUserObject['consultantUserId'], $loggedInUserObject['id'], "");    
      updateReceiptKeyValue($receipt['x_trans_id'], "itemCreated", 1);
      }
    }
...
function attemptAcceptItem($itemId){
  ...
  if($item['inviteEmail'] != ""){
    if(strtolower($item['inviteEmail']) == strtolower($loggedInUserObject["email"])){
      ...
      $loggedInUserObject['proceedToItemId']=$itemId;
      // so update item
...
function returnOpenUserReceiptsOLD(){
  global $loggedInUserObject;
  global $mysqli;
...
function returnOpenUserReceipts(){
  global $loggedInUserObject;
  global $mysqli;
  $iqString="SELECT products.*, receipts.orderID, receipts.name, receipts.sku FROM receipts INNER JOIN products ON receipts.sku = products.sku WHERE receipts.itemCreated = 0 AND receipts.email1 = '".$loggedInUserObject['email']."'";
  $iqResult = $mysqli->query($iqString);
  $resultArray=array();
  if($iqResult !== false ) { 
    while ($record = $iqResult->fetch_assoc()) {
...
function returnOpenReceipts(){
  global $loggedInUserObject;
  global $mysqli;
  global $sessionIsAdmin;
  $resultArray=array();
  if($sessionIsAdmin==1){ // admin only sees open receipts
    $iqString="SELECT products.*, receipts.* FROM receipts INNER JOIN products ON receipts.x_catalog_link_id = products.catalogLinkId WHERE receipts.itemCreated = 0 AND receipts.x_response_code = 1";
    $iqResult = $mysqli->query($iqString);
...
function returnItemWhere($key, $value){
  global $mysqli;
  $iqString="SELECT items.*, products.name, products.marketingUrl, products.description, products.productType, products.disposition, products.contentRequiresAssessed FROM items INNER JOIN products ON items.productId = products.id WHERE items.".$key." = '".$value."'";
  $iqResult = $mysqli->query($iqString);
  $resultArray=array();
  if($iqResult !== false ) { 
    while ($record = $iqResult->fetch_assoc()) {
...
function returnUnusedGiftsOfUser($userId){
  global $mysqli;
  global $debugStack;
  $iqString="SELECT items.*, products.name, products.marketingUrl, products.description, products.productType, products.disposition FROM items INNER JOIN products ON items.productId = products.id WHERE consumerUserId = 0 AND inviteEmail = '' AND consultantUserId = 0 AND creatorUserId = " . $userId;
...
function returnUnusedInvitesOfConsultant($consultantUserId){
  global $mysqli;
  $iqString="SELECT items.*, products.name, products.marketingUrl, products.description, products.productType, products.disposition FROM items INNER JOIN products ON items.productId = products.id WHERE consumerUserId = 0 AND inviteEmail = '' AND consultantUserId = " . $consultantUserId . " AND products.disposition = 'invite'";
  $iqResult = $mysqli->query($iqString);
  $resultArray=array();
  if($iqResult !== false ) { 
    while ($record = $iqResult->fetch_assoc()) {
...
function returnPendingInvitesOfConsultant($consultantUserId){
  global $mysqli;
  $iqString="SELECT items.*, products.name, products.marketingUrl, products.description, products.productType, products.disposition FROM items INNER JOIN products ON items.productId = products.id WHERE consumerUserId = 0 AND inviteEmail != '' AND consultantUserId = " . $consultantUserId;
  $iqResult = $mysqli->query($iqString);
  $resultArray=array();
  if($iqResult !== false ) { 
    while ($record = $iqResult->fetch_assoc()) {
...
function returnUsersWhereFilter($filter){
  ...
  $users=array();
  if($filter != ""){
    $qString="SELECT * FROM users WHERE wpLogin LIKE '%".$filter."%' OR firstName LIKE '%".$filter."%' OR lastName LIKE '%".$filter."%' ";
    }
  $result = $mysqli->query($qString);
  if($result !== false ) { 
    while ($record = $result->fetch_assoc()) {
