...
function modalProduct(id){
  ...
  for(var p=0; p<products.length; p++){
    if(products[p].id==id){
      ...
      wizUrl=products[p].automationWizUrl;
  ...
  //dbuga("linkId="+linkId);
  var htmlStr="";
  htmlStr+='<div class="whiteMessage">Logged in as '+user.wpDisplayName+invalidMessage;

  htmlStr+='</div>';
  if(isValid){
    htmlStr+='<div class="modalImageForm">'+returnImageForm(linkId, wizUrl, sku)+'</div>';
    }
    ...
    if(theArray[p]["disposition"]==""){
      if(productIsValid(theArray[p])){
        if(showBuyNow){
          ...
          rowString='<tr><td colspan=2>'+returnImageForm(theArray[p].catalogLinkId, theArray[p].automationWizUrl, theArray[p].sku)+'</td></tr>';
          }
        }
      else{
...
function returnImageForm(linkId, wizUrl, sku){
  var formStr='';
  if((user.id==69)||(user.id==77)){
    formStr+='<a href="'+wizUrl+'"><img src="/webapp/buynows/'+sku+'.png" class="authorizeImageButton" /></a>';
    }
  else{
    formStr=+'<form name="PrePage" method = "post" action = "https://Simplecheckout.authorize.net/payment/CatalogPayment.aspx"> <input type = "hidden" name = "LinkId" value ="'+linkId+'" /> <input type="image" src="/webapp/buynows/'+sku+'.png" alt="submit" class="authorizeImageButton" /> </form>';
    }
  return formStr;
...
function leftNav(e){
  ...
  if(leftnavButtons[activeNavButton].label=="help_icon_red.png"){
    window.open("http://www.support.corepassion.com/", "_blank");
    }
  }
