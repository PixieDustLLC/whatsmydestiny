...
function init(){
  ...
  app("appDiv", '<div id="downloadpdf" style="display:none;"><a id="pdfData" href="" target="pdf"><img src="webapp/nav/downloadpdf.png"/></a></div>');
...
function divScrolled(theDiv){
  ...
  canv.height=appSquarePx;
  var proportion=a.offsetHeight/a.scrollHeight;
  var barHeight=appSquarePx*proportion;
  var barRange=appSquarePx-barHeight;
  var scrollRange=a.scrollHeight-a.offsetHeight;
  var progress=a.scrollTop/scrollRange;
  ...
  ctx.lineWidth=g;
  ctx.beginPath();
  ctx.moveTo(g+g/2, barTop+g*1.5);
  ctx.lineTo(g+g/2, barTop+barHeight-g*1.5);
  ctx.stroke();
...
function preloadImages(){
  for(var b=0; b<leftnavButtons.length; b++){
    var label=leftnavButtons[b].label;
    if(label.indexOf('.png')>-1){
      preloadImage('webapp/icons/'+label);
      }
    }

  for (var c=0; c<tileColors.length; c++){
    preloadImage('webapp/tiles/tile_'+tileColors[c]+'.png');
    }
  for (var b=0; b<badges.length; b++){
    preloadImage('webapp/tiles/badge_p_1_'+badges[b]+'_s.png');
    }
...
function preloadImage(url){
  ...
  img.onload=function(){
    ...
    if(preloaded==preloading){
      console.log("loaded "+preloaded+" of "+preloading);
      drawLeftnav();
      }
    };
  img.src = url;
...
function modalProduct(id){
  ...
  var htmlStr="";
  htmlStr+='<div class="whiteMessage">Welcome '+user.wpDisplayName+invalidMessage+'</div>';
  if(isValid){
...
function recoverString(str){
  // should accept string with contents of [olduserid].txt
  var recoverArray=JSON.parse(str);
  var recoverObject=recoverArray[0];
...
function productIsValid(product){
  ...
  if((product.dev=="1")&&(userIsDev==true)){isValid=true;}
...
function checkGeometry(){
  ...
  var  sixteenPt=16*g/5.1;
    ...
    style.innerHTML += '.appSquare h1{color:#fff; width:'+g*52+'px; text-align:center; line-height:'+g*6+'px; font-size:'+fontPx*1.25+'px; height:'+g*5+'px; float:right; margin-top:'+g*2+'px; margin-bottom:'+0+'px; Zbackground-color:#8c13d9; }';
    style.innerHTML += '.appSquare h2{color:#000; width:'+g*58+'px; text-align:center; line-height:'+g*6+'px; font-size:'+fontPx*1.25+'px; height:'+g*5+'px; float:right; margin-top:'+g*0+'px; margin-right:'+g*3+'px; background-color:#8c13d9;  margin-bottom:0;}';
    ...
    style.innerHTML += '.itemsTable{float:right; padding:'+g*2+'px; width:'+g*58+'px; margin-top:'+0+'px; margin-right:'+g*3+'px; background-color:white;  margin-bottom:0; background-color:white; color:black; font:'+twelvePt+'px openSansSemiBold;}';
  ...
  //red ba1c1b
    ...
    style.innerHTML += '.appSquare{width:'+g*61+'px; height:'+appSquarePx+'px; background-color:black;}';

      
    style.innerHTML += '#actionScroll, #contextScroll{position:absolute; height:'+appSquarePx+'px; width:'+g*3+'px; background-color:black; }';
    style.innerHTML += '#actionScroll{left:'+(actionLeft+g*61)+'px; top:'+actionTop+'px;}';
    style.innerHTML += '#contextScroll{left:'+(appLeftPad+g*61)+'px; top:'+appTopPad+'px;}';

    style.innerHTML += '.shade{width:'+g*61+'px; height:'+g*2+'px; position:absolute;}';
    ...
    style.innerHTML += '#leftnavContext{left:'+appLeftPad+'px; top:'+appTopPad+'px; zbackground: linear-gradient(to left, rgba(255,255,255,.75),rgba(255,255,255,.75),rgba(255,255,255,.5)); width:'+Math.ceil(g*8)+'px; height:'+Math.ceil(g*8*leftnavButtons.length)+'px; position:absolute;}';
...
*/
    ...
    style.innerHTML += '#actionDiv{left:'+actionLeft+'px; top:'+actionTop+'px;  overflow:scroll;}';
    ...
    style.innerHTML += '.tile{background-size:contain; width:'+g*26+'px; height:'+g*26+'px; float:right;}';
    style.innerHTML += '.badge{background-size:contain; width:'+g*24+'px; height:'+g*13.5+'px; margin:'+g+'px; float:right;}';
    style.innerHTML += '.tileText{width:'+g*26+'px; height:'+g*4+'px; margin:'+g*0+'px; float:right; font-size:'+g*2+'px; text-align:center;}';
    style.innerHTML += '.tileButton{ser-select:none; background-color:rgba(255,255,0,0); width:'+g*26+'px; height:'+g*6+'px; line-height:'+g*6.5+'px; margin:'+g*0+'px; float:right; font-size:'+g*2+'px; text-align:center;}';

    document.getElementsByTagName('head')[0].appendChild(style);
    document.getElementById('actionScroll').width=Math.floor(g*3)+1;
    document.getElementById('actionScroll').height=appSquarePx;
    document.getElementById('contextScroll').width=Math.floor(g*3)+1;
    document.getElementById('contextScroll').height=appSquarePx;
...
var tileColors=["white", "grey", "red", "blue", "green", "yellow"];
var textColors=["black", "white", "white", "white", "white", "black"];
var tileText=["Registration complete", "Career Seeker synced", "Now available", "Support@corepassion.com", "Unopened Item", "40% completed"];
var tileButton=["INFORMATION", "REPORTS", "BUY NOW", "CONTACT", "BEGIN", "RESUME"];
var badges=["cpa", "int2016diy", "int2016diy-cpa", "int2016jan", "int2016jan-cpa", "msmpva"];

function randomTiles(){
  var tiles='<h1>My Core Passion®</h1>';
  var tNum=1+rnd(12);
  for(var t=0; t<tNum; t++){
    var colorNum=rnd(tileColors.length)
    var color=tileColors[colorNum];
    var badge=badges[rnd(badges.length)];
    tiles+='<div class="tile" style="background-image:url(webapp/tiles/tile_'+color+'.png);">';

    tiles+='<div class="badge" style="background-image:url(webapp/tiles/badge_p_1_'+badge+'_s.png);">';
    tiles+='</div>';

    tiles+='<div class="tileText" style="color:'+textColors[colorNum]+';">'+tileText[colorNum];
    tiles+='</div>';

    tiles+='<div class="tileButton" style="color:'+textColors[colorNum]+';" onmousedown="console.log(\''+tileButton[colorNum]+'\')">'+tileButton[colorNum];
    tiles+='</div>';
...
var leftnavButtons=[{"label":"icon-house.png"},{"label":"icon-cart.png"},{"label":"icon-chat.png"},{"label":"icon-user.png"}];
...
function drawLeftnav(){
  ...
  canv.width=Math.ceil(g*8);
  canv.height=Math.ceil(g*8)*leftnavButtons.length;
  var ctx=canv.getContext('2d');
  for(var b=0; b<leftnavButtons.length; b++){
    var pad=g/8;
    var x=0+pad*2;
...
style.innerHTML += '#appWordmark{width:'+g*58+'px; height:'+g*6+'px;  float:right; margin-top:'+g+'px; background-color:#8c13d9;}';
