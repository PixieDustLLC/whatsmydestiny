...
function returnItemWhere($key, $value){
  global $mysqli;
  $iqString="SELECT items.*, products.name, products.productType, products.disposition, products.contentRequiresAssessed FROM items INNER JOIN products ON items.productId = products.id WHERE items.".$key." = '".$value."'";
  $iqResult = $mysqli->query($iqString);
  $resultArray=array();
  if($iqResult !== false ) { 
    while ($record = $iqResult->fetch_assoc()) {
...
function returnItemsWhere($key, $value){
  ...
  $iqString="SELECT items.*, products.name, products.deliverable, products.linkUrl, products.productType, products.disposition, products.contentRequiresAssessed FROM items INNER JOIN products ON items.productId = products.id WHERE items.".$key." = '".strtolower ($value)."'";
  $iqResult = $mysqli->query($iqString);
  $resultArray=array();
  if($iqResult !== false ) { 
    while ($record = $iqResult->fetch_assoc()) {
