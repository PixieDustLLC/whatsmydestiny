...
function manageProfile(){
  ...
  if(user.isConsultant==1){
    htmlString+='<span style="clear:both;"></span><input type="button" class="navButton" onclick="buttonDown(this.id); proceedToItemId=0; showProducts();" id="navProfileCancel" value="cancel" /><input type="button" class="navButton" onclick="buttonDown(this.id); updateConsultantProfile()" id="navProfileUpdate" value="update" />';
    }
  else{
...
function grantSelection(selection){
  ...
  jQuery.post(appPath+"ajaxsql.php", serialForm, function(data, textStatus) {
    ...
    if(returnedErrors.length>0){
      ...
      if(user.messages.length>0){
        var actionString='<div class="messages"><p>';
        actionString+=user.messages.join("<br />")+'</p></div>';
        app("actionDiv", actionString);
        }
...
function checkGeometry(){
  ...
  //red a5100c
    style.innerHTML += '.appSquare .userSelect select {width:'+(gridPx*8-10)+'px!important; height:'+(gridPx*1.5)+'px!important; margin-left:'+0+'px; margin-top:'+g*0+'px; font-size: '+fontPx*.9+'px; line-height: 1; border: 0; border-radius: 0; -webkit-appearance: none; background: transparent; float:left;}';
    style.innerHTML += '.appSquare .userSelect{width:'+(gridPx*8)+'px!important; height:'+(gridPx*1.5)+'px!important; margin:'+gridPx/4+'px; overflow: hidden; background: url(webapp/down_arrow_select.jpg) no-repeat right #ddd; border: 1px solid #ccc;}';
    ...
    style.innerHTML += '.tile{background-size:contain; width:'+g*26+'px; height:'+g*26+'px; margin:'+g/2+'px '+g/2+'px 0 0; position:relative;  float:right; font-family:openSansSemiBold;}';
    ...
    style.innerHTML += '.tileImage{top:'+g*2.5+'px; background-size:contain; height:'+g*13+'px;}';
...
function selectBucketProduct(){
  console.log('isGrant '+ document.bucketForm.isGrant.value);
  ...
  if(document.bucketForm.isGrant.value=="false"){
    ...
    var targetUserId=Number(document.bucketForm.grantUserSelect.value);
    if(targetUserId==0){
...
function grantProductToUser(productId, userId){
  ...
  if(parts[3]=="g"){disposition="gift";}//no gifts active anyways
  if(parts[3]=="i"){disposition ="invite";}
...
function tileClick(linkGlom){
  ...
  if(linkGlom.indexOf('bucket_')==0){
    ...
    if(grants>0){
      ...
      formString+='<div class="userSelect"><select id="grantUserSelect" placeholder="Select User">';
      formString+='<option value="0" disabled SELECTED>Select User</option>';
      var ids=idsOfAll();
      for (var u=0; u<ids.length; u++){
        formString +='<option value="'+ids[u]+'">'+usersById[ids[u]].wpDisplayName +'</option>';
        }
...
function itemMore(id){
  ...
  if(item.marketingUrl != ""){
    infoString='<div class="infoButton" style="float:right; width:'+g*6+'px; height:'+g*6+'px;" onclick="window.open(\''+item.marketingUrl+'\', \'_blank\')">&nbsp;</div>';
    }
  ...
  if(showCpaLink){
    aString+='<input class="navButton" style="float:right;" type="button" onclick="showCpa(\''+id+'\')" value="'+cpaLinkText+'" />';
    }
  if(showPrioritizeLink){
    aString+='<input class="navButton" style="float:right;" type="button" onclick="prioritizeItemId(\''+id+'\')" value="SELECT" />';
    }
  if(showReportLink){
    aString+='<input class="navButton" style="float:right;" type="button" onclick="pdfReportItemId(\''+id+'\')" value="REPORT" />';
    }
  aString+='</p>';
  aString+='<hr />';
  if(showContentLink){
    aString+='<p><input class="navButton" style="float:right;" type="button" onclick="window.open(\''+item.linkUrl+'\', \'_blank\')" value="OPEN" />Resources:</p><p style="clear:both;">'+item.linkUrl+'</p>';
    }

  aString+='<div style="margin-right:'+g*2+'px; margin-top:'+g*0+'px; width:'+g*50+'px; height:'+g*25+'px; display:block; float:right; Zbackground-color:blue; background-image:url('+imageUrl+'); background-size:contain;">&nbsp;</div>';
...
function cpaGeometry(){
  ...
  style.innerHTML += '.infoButton{padding:0px; background-image:url(/webapp/icons/ir-question-88p.png); background-size:contain; display:inline-block; width:'+g*4+'px; height:'+g*4+'px;}';
...
function returnActiveSortArray(){
  ...
  var colorArray=["#ffff00", "#ff3d00", "#ff7e00", "#ffbe00", "#feff00", "#767676", "#767676", "#767676", "#767676", "#767676", "#767676", "#767676"];
  var sortArray=[];
  for (var p=0; p<passions.length; p++){
