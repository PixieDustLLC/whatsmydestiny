...
function refreshUsersTables(){
  ...
  if(user.isAdmin==1){
    showObject={"wpDisplayName":6, "isConsultant":3, "consultantUserId":5};
...
function tableFromArrayOfItems(theArray, showObject){
  ...
  for (var u=0; u<theArray.length; u++){// this should be i not u
    ...
    for(var k=0; k<keys.length; k++){
      ...
      if(inArray(keys[k], showArray)){
        if(keys[k]=="name"){
          //outputString='<a href="http://corepassion.com/?page_id=66#action=accept&id='+theArray[u]['id']+'">'+outputString+'</a>';
          if(theArray[u]['completed']==0){
            if(theArray[u]['consumed']==0){
              if(theArray[u]['consumerUserId']==0){// pending, no link
                }
              else{
                outputString='<a href="javascript:showCpa(\''+theArray[u]['id']+'\')">Begin '+outputString+'</a>';
                }
              }
            else{
              outputString='<a href="javascript:showCpa(\''+theArray[u]['id']+'\')">Resume '+outputString+'</a>';
              }
            }
...
function returnTablesFromArrayOfUsers(theArray, showObject){
  ...
  for(t=0; t<types.length; t++){
    if(usersByType[types[t]].length>0){
      tables+='<h4>'+usersByType[types[t]].length+" "+types[t]+'</h4>';
...
function checkGeometry(){
  ...
  var  sixteenPt=16*g/5.1;
    ...
    style.innerHTML += '.usersTable{padding:'+g*2+'px; width:'+g*58+'px; margin-left:'+g*3+'px; background-color:white; color:black; font:'+twelvePt+'px openSansSemiBold;}';
...
function showTeamProfile(teamProfile){
  ...
  var data = canv.toDataURL('image/jpeg').slice('data:image/jpeg;base64,'.length);
  // Convert the data to binary form
...
function resumePdfReportItemId(itemId){
  ...
  var data = destCanv.toDataURL('image/jpeg').slice('data:image/jpeg;base64,'.length);
  // Convert the data to binary form
  var imgData = atob(data);
...
function imageToDoc(doc, id, gx, gy, gw, gh){
  ...
  var scaledWidth=imageDensity*gw;
  var scaledHeight=imageDensity*gh;
  ...
  var data = tempCanv.toDataURL('image/jpeg').slice('data:image/jpeg;base64,'.length);
  // Convert the data to binary form
  var imgData = atob(data);
