...
function preloadImage(url){
  ...
  img.onload=function(){
    ...
    if(preloaded==preloading){
      console.log(" loaded "+preloaded+" of "+preloading+" so checkGeometry");
      checkGeometry();
...
function ajaxLogin(){
  ...
  jQuery.post(appPath+"ajaxsql.php", serialForm, function(data, textStatus) {
    ...
    if(returnedErrors.length>0){
      ...
      console.log("proceedToItemId="+proceedToItemId+" typeof=")
...
function sessionLog(){
  ...
  if((hashId=="")||(hashAction=="")){
    ...
    document.getElementById('appDiv').style.display="block";
    console.log('reveal appDiv to login');
    document.getElementById('appDiv').style.opacity="1";
  ...
  jQuery.post(appPath+"ajaxsql.php", serialForm, function(data, textStatus) {
    document.getElementById('appDiv').style.display="block";
    console.log('login show appDiv fail 4');
    document.getElementById('appDiv').style.opacity="1";
    rep("appDiv", '<h2>Please <a href="http://www.corepassion.com/?page_id=56">Log In</a> or <a href="http://www.corepassion.com/?page_id=58">Register</a></h2>');
    }, "json");
  }
...
function itemMore(id){
  ...
  aString+='<hr />';
  if(showContentLink){
    aString+='<p><input class="itemMoreButtons" style="float:right;" type="button" onclick="window.open(\''+item.linkUrl+'\', \'_blank\')" value="RESOURCES" /></p>';
    }
...
// recently imported
  ...
  // itemMore
  style.innerHTML += '.itemMoreProductImage{margin-right:'+g*2+'px; margin-top:'+g*0+'px; width:'+g*26+'px; height:'+g*13+'px; display:block; float:left;   background-size:contain;}';
  style.innerHTML += '';
