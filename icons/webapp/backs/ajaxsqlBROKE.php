...
if(count($errorStack)==0){// can return data
  ...
  if($action=="login"){
    ...
    if(($wpUserMeta['wpUserDisplayName'] != $loggedInUserObject['wpDisplayName'])||($wpUserMeta['wpUserFirstName'] != $loggedInUserObject['firstName'])||($wpUserMeta['wpUserLastName'] != $loggedInUserObject['lastName'])){
      ...
      $mysqli->query($qString) or die("user profile not updated? ".$qString);
      }
  ...
  if($action=="updateConsultantProfile"){
    ...
    $qString.="`inviteText` = '".$_POST['inviteText']."'";
    $qString.=" WHERE id = ".$sessionUserId;
    ...
    $loggedInUserObject['inviteText']=$_POST['inviteText'];
    $loggedInUserObject['ethnicity_id']=$_POST['ethnicity_id'];
    $loggedInUserObject['gender_id']=$_POST['gender_id'];
    $loggedInUserObject['age_id']=$_POST['age_id'];
    $loggedInUserObject['career_id']=$_POST['career_id'];
...
*/
  ...
  if($action=="unhideItemIds"){
    ...
    updateItemsKeyValue($_POST['itemIds'], 'isBlind', 0);
  ...
  if($action=="deleteUnusedItems"){
    ...
    if($deleteNumber <= unusedCount){
      array_push($messagesStack, '"valid '.$deleteNumber. ' <= '.$unusedCount.'"');
      }
    else{
  ...
  if($action=="sendInvite"){
    ...
    foreach($inviteEmails as $inviteEmail){
      ...
      if(@mail($inviteEmail, $subject, $message, $headers)){
        ...
        updateItemKeyValue($item['id'], "inviteEmail", $inviteEmail);
    ...
    if(count($itemArray)>0){
      ...
      if($item['consumerUserId']==$sessionUserId){
        ...
        updateItemKeyValue($_POST['itemId'], "completed", $_POST['completed']);
          ...
          updateItemKeyValue($_POST['itemId'], "ethnicity_id", $_POST["ethnicity_id"]);
          updateItemKeyValue($_POST['itemId'], "gender_id", $_POST["gender_id"]);
          updateItemKeyValue($_POST['itemId'], "age_id", $_POST["age_id"]);
          updateItemKeyValue($_POST['itemId'], "career_id", $_POST["career_id"]);
...
function updateItemsKeyValue($ids, $key, $value){
  global $mysqli;
  global $debugStack;
  array_push($debugStack, '"UPDATE items SET '.$key.'"');
  $qString="UPDATE items SET ".$key." = '".$value."' WHERE id IN (".$ids.")";
...
function returnItemsWhere($key, $value){
  global $mysqli;
  $iqString="SELECT items.*, products.name, products.productType, products.disposition FROM items INNER JOIN products ON items.productId = products.id WHERE items.".$key." = '".strtolower ($value)."'";
  $iqResult = $mysqli->query($iqString);
  $resultArray=array();
  if($iqResult !== false ) { 
    while ($record = $iqResult->fetch_assoc()) {
