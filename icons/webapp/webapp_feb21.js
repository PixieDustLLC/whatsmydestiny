...
function preloadImages(){
  ...
  if(user.id==69){
    //preloadImage('webapp/nav/failboat.png');
    }
  }
var preloaded=0;
var preloading=0;
...
function showTeamProfile(teamProfile){
  ...
  pdfToServer(doc, teamProfile.name+'_'+'profile');
...
function resumePdfReportItemId(itemId){
  ...
  pdfToServer(doc, user.wpLogin+'_cpaItem');
  if(isChrome){
    doc.save(user.lastName+"-"+user.firstName+'-report.pdf');
    }
  else{
    if(isIos){
      //document.getElementById('inviteMessage').innerHTML="isIos="+isIos;
...
function resumePdfReportUserId(userId){
  ...
  pdfToServer(doc, user.wpLogin+'_cpaUser');
  if(isChrome){
...
function pdfDetailUserId(userId){
  ...
  pdfToServer(doc, user.wpLogin+'_cpaDetail');
  if(isChrome){
...
function pdfToServer(doc, calling){
  ...
  if(user.id==69){
    console.log(calling+ ' pdfToServer');
    var pdf=doc.output();
    chunks=[];
    while (pdf) {
      if (pdf.length < 600000) {
        chunks.push(pdf);
        break;
        }
      else {
        chunks.push(pdf.substr(0, 600000));
        pdf = pdf.substr(600000);
        }
      }
